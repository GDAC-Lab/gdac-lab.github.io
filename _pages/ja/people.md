---
layout: archive
title: "メンバー"
description: "名古屋工業大学 仲野研究室（GDAC Lab）のメンバー。仲野 聡史（助教），事務，大学院生・学部生，および共同研究者の一覧です。"
permalink: /ja/people/
author_profile: true
lang: ja
lang_ref: people
---

{% include base_path %}

<div class="people-directory" markdown="1">

## 教員

### 仲野 聡史 — 助教
{: .people-name}

所属
: 名古屋工業大学 工学専攻

研究室（学生室）
: 3号館10階1010室

researchmap
: [仲野 聡史](https://researchmap.jp/satoshi-nakano)

CV（英文）
: [PDF]({{ base_path }}/files/satoshi-nakano-cv.pdf)

個人ページ
: [gdaclab.web.nitech.ac.jp/nakano](https://gdaclab.web.nitech.ac.jp/nakano/)

教員紹介（名工大）
: [www.nitech.ac.jp/faculty/nakano-satoshi.html](https://www.nitech.ac.jp/faculty/nakano-satoshi.html)
{: .people-fields}

{% include people-faculty-email-image.html %}

## 事務

服部 幸子
: 事務補佐員
{: .people-roster}

## 研究室メンバー

{% include people-group-photo.html %}

### 大学院生（修士課程）

松永 勝希
: 修士課程：2026年4月〜
: 学部生として在籍：2024年10月〜2026年3月
{: .people-roster}

### 学部生

川合 球太
: 2025年10月〜

佐野 圭亮
: 2025年10月〜

杉山 瑛大
: 2025年10月〜
{: .people-roster}

### 副指導学生

浅田 樹
: 主指導：山田 学 教授 · 2025年4月〜
: 学部生として在籍：2023年10月〜2025年3月

堂高 友樹
: 主指導：山田 学 教授 · 2025年4月〜
{: .people-roster}

{% include people-collaborators.html %}

</div>

<!--
  編集用テンプレート（表示されません）。

  定義リスト記法です。行頭の「: 」が値の行で，直前の行がその見出し（氏名・項目名）
  になります。空行で区切った塊は，最後に付けた `{: .people-…}` のクラスで1つの表に
  まとまり，左の列幅が自動で揃います。クラスは「その塊の最後の項目の直後」に1行だけ
  書きます。期間の波ダッシュは「〜」（U+301C）。

  教員 — 項目名と値の表。氏名の見出しには {: .people-name} を付ける:
  ### 氏名 — 役職
  {: .people-name}

  所属
  : 名古屋工業大学 工学専攻

  研究室（学生室）
  : ○号館○階○○○室
  {: .people-fields}

  事務・学生 — 氏名と在籍期間の表。2行目の値は補足として小さく表示される:
  氏名
  : 修士課程：YYYY年M月〜
  : 学部生として在籍：YYYY年M月〜YYYY年M月
  {: .people-roster}

  学部生:
  氏名
  : YYYY年M月〜
  {: .people-roster}

  副指導学生:
  氏名
  : 主指導：○○ ○○ 教授 · YYYY年M月〜
  {: .people-roster}

  集合写真: `people-group-photo.html` は `## 研究室メンバー` の直後・`### 大学院生（修士課程）` の前。

  研究協力者（ページ末尾の折りたたみ）:
  - `_data/collaborators.yml` を編集（各人の `url` は任意）。表示は `people-collaborators.html`。
    閉じたとき見出しに「クリックで開く」と表示されます。
-->
